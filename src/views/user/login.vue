<template>
  <div class="login-box">
    ...
    <div class="login-body">
      ...
      <div v-show="status==1" @click="inputFocus" style="position: relative">
        <div v-show="!useMobile" class="input-mask"></div>
        <div class="pwd-box">
      ...
      <div v-show="status==4" class="status-4-box">
        <div class="pwd-box">
          <input v-model="password" aria-required="true" required="required"  type="password" maxlength="20"  class="login-input" :placeholder="findPwd?'6-12位大小写数字字母' : '6-12位大小写数字字母'">
          <i class="toggle-pwd" @click="togglePwd"></i>
        </div>
        <div class="sep-line"></div>
      ...
      <div v-show="status==7 || status == 9" class="status-7-box">
        <div class="pwd-box">
          <input id="pwd-input" v-model="password"  aria-required="true" required="required" type="password"  maxlength="20" class="login-input" placeholder="请输入密码">
          <i class="toggle-pwd" @click="togglePwd"></i>
        </div>
        <div class="sep-line"></div>
    ...
    <third-login @chooseMobile="chooseMobile" :showMobile="status ==1" v-show="!modPwd && !modPhone && (status ==1 ||status==9)" class="third-box" :class="{'no-mobile':status ==1}"></third-login>
  </div>
</template>
...
<script>
  ...
  export default {
    data () {
      return {
        useMobile: false, // 默认不显示手机登录，突出第三方登录
        mobile: null, // 手机号
        showNext: false, // 是否显示下一步按钮
        showNavLeft: true, // 是否显示后退按钮
        msg: '欢迎来到千呼旅行', //头像下方提示文字
        status: 2, // 当前状态
        lastStatus: 0,
        dragNum: 20,
    ...
    watch: {
      'status': function (v) {
        if (v == 10) {
          // alert("跳转isPersonal")
          // this.$router.push({path: '/account/no-apply'})  后台返回跳转路径
          this.$router.push({path: '/isPersonal'})
        }
      },
      ...
      'mobile': function (v) {
        if(!this.modPwd && !this.modPhone && /^1[34578]\d{9}$/.test(v)){
          this.phoneExist()
//          if (v == '15755555555') {
...
//          }
    ...
    mounted () {
      ...
      if (this.$route.path.indexOf('modPwd') > -1) { // 个人中心的修改密码页面
        this.modPwd = true
        this.mobile = this.$store.getters.userInfo.phone
        this.status = 11
      }
    ...
    methods: {
      ...
      chooseMobile () {
        ...
        if (this.useMobile) $('#login-mobile').focus()
      },
      /**
       * 密码登录
       * */
      loginByPwd () {
        this.$ajax.post('/customer/login', {
          ...
          this.$router.push({path: this.$methods.judgeRoute(data.audit)})
        })
      },
      /**
       * 判断手机号有无注册
      ...
      forgotPwd () {
        this.findPwd = true
        this.changeStatus(2, this.status)
      },
      /**
       * 检验两次密码输入是否一致
       * */
      ...
      backStatus () {
        ...
        if (this.status > 9) this.$router.go(-1)
      ...
      changeStatus (v, o) {
        if(v=='5'&&o=='4'){
          var reg=new RegExp('^(?![a-zA-Z]+$)(?![A-Z0-9]+$)(?![A-Z\W_!@#$%^&*`~()-+=]+$)(?![a-z0-9]+$)(?![a-z\W_!@#$%^&*`~()-+=]+$)(?![0-9\W_!@#$%^&*`~()-+=]+$)[a-zA-Z0-9\W_!@#$%^&*`~()-+=]{6,12}$');
          if(!reg.test(this.password)){
            window.HMO_APP.$vux.toast.show({
              text: "密码格式不正确,要同时有大小写字母和数字，6到12个字母"
            })
            this.password="";
            return
          }
