...
(function ($, window, document, undefined) {
  ...
  function readyDom(iObj) {
    ...
    holder.html(html.join('')).css({
      'width': (matrix[1] * (radius * 2 + margin * 2)) + 'px',
      'height': (matrix[0] * (radius * 2 + margin * 2)) + 'px'
    });
      ...
      if (idx) {
        if (patternAry.indexOf(pattId) == -1) {
          ...
          var margin = iObj.option.margin,
            ...
            newX = (posObj.i - 1) * (2 * margin + 2 * radius) + margin + radius,
            newY = (posObj.j - 1) * (2 * margin + 2 * radius) + margin + radius;
          ...
          //to create new line
          var line = $('<div class="patt-lines" style="top:' + (newY) + 'px; left:' + (newX) + 'px"></div>');
          iObj.line = line;
          iObj.lineX1 = newX;
          iObj.lineY1 = newY;
          //add on dom
  ...
  InternalMethods.prototype = {
    ...
    getIdxFromPoint: function (x, y) {
      ...
      if (qsntX <= matrix[1] && qsntY <= matrix[0] && remX > (margin * 2 + 10) && remY > (margin * 2  + 10)) {
